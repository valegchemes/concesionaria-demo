...
const STATUS_CONFIG: Record<StatusType, { label: string; bg: string; text: string; icon: any }> = {
  PENDIENTE: { label: 'Pendiente', bg: 'bg-red-50 hover:bg-red-100/70 border-red-200 text-red-700', text: 'text-red-700', icon: AlertCircle },
  TRAMITE: { label: 'En Trámite', bg: 'bg-amber-50 hover:bg-amber-100/70 border-amber-200 text-amber-700', text: 'text-amber-700', icon: Clock },
  COMPLETO: { label: 'Completo', bg: 'bg-emerald-50 hover:bg-emerald-100/70 border-emerald-200 text-emerald-700', text: 'text-emerald-700', icon: ShieldCheck },
  NO_APLICA: { label: 'No Aplica', bg: 'bg-slate-50 hover:bg-slate-100/70 border-slate-200 text-slate-500', text: 'text-slate-500', icon: HelpCircle },
}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="md:col-span-2 border border-border">
          <CardContent className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div className="space-y-1.5 flex-1">
              <span className="text-xs uppercase font-bold text-slate-400 tracking-wider">Control de Trámites</span>
              <h3 className="text-lg font-bold text-slate-800">Progreso Legal de la Unidad</h3>
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Completado</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="flex flex-col justify-center border border-border">
          <CardContent className="p-6 space-y-3 flex flex-col justify-center h-full">
            ...
            {saveSuccess && (
              <p className="text-center text-xs font-semibold text-emerald-600 animate-fade-in flex items-center justify-center gap-1">
      ...
      <Card className="border border-border">
        ...
        <CardContent className="p-0 divide-y divide-border">
          {GESTORIA_ITEMS.map((item) => {
            ...
            return (
              <div key={item.key} className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 hover:bg-slate-50/50 transition-colors">
                {/* Text explanation */}
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded-lg border text-xs font-semibold flex items-center gap-1.5 transition-all ${
                          isSelected 
                            ...
                            : 'bg-background hover:bg-slate-50 border-border text-muted-foreground hover:text-slate-800'
                        }`}
                      >
